...
def get_fare(origin, destination):
    """Get the fare $ amount from the BART API"""
    ...
    r = requests.get("http://api.bart.gov/api/sched.aspx?cmd=fare", params=params)

    fare = r.json()

    return fare.get('root').get('fares').get('fare')[0].get('@amount')
...
def get_job_results(selected_station, title, within_age):
    ...
    if selected_station == 'all':
        ...
        job_results = (db.session.query(Job.url,
                                        ...
                                        Bart.station_latitude,
                                        Bart.station_longitude,
                                        Bart.station_code)
                        .join(Business)
                        .join(Bart)
